...
- `lib/klenod/runtime/`: runtime bundle loading and module evaluation. Runtime must not require build plugins or heavyweight build dependencies.
- `lib/klenod/http/`: optional serving helpers, not framework policy.
...
An `ARCHITECTURE.md` would be useful next. It should probably summarize graph states, plugin phases, runtime boundaries, router behavior, and asset flow in one stable design document.
...
The example server uses `async-http`. It logs requests with ANSI formatting and dims asset requests.
