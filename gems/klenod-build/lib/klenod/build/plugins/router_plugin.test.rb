...
class Klenod::Build::Plugins::RouterPlugin::Test < Minitest::Test
  RouterPlugin = Klenod::Build::Plugins::RouterPlugin::Plugin
  ...
  def test_ignores_old_non_prefixed_route_file_names
    Dir.mktmpdir do |dir|
      FileUtils.mkdir_p("#{dir}/pages")
      File.write("#{dir}/pages/page.rb", "NAME = :old_page\n")
      File.write("#{dir}/pages/route.rb", "def GET(_req)\n  :old_route\nend\n")
      File.write("#{dir}/pages/layout.rb", "NAME = :old_layout\n")
      File.write("#{dir}/pages/error.rb", "NAME = :old_error\n")
      File.write("#{dir}/pages/not-found.rb", "NAME = :old_not_found\n")

      manifest = RouterPlugin.new.discover(source_dir: dir)

      assert_equal([], manifest.routes)
      assert_equal([], manifest.special_views)
    end
  end

  def test_raises_for_unsupported_plus_router_files
    Dir.mktmpdir do |dir|
      FileUtils.mkdir_p("#{dir}/pages")
      File.write("#{dir}/pages/+page.haml", "")
      File.write("#{dir}/pages/+foo.haml", "")
      File.write("#{dir}/pages/+bar.rb", "")
      File.write("#{dir}/pages/+page.svg", "")
      ...
      assert_includes(error.message, "Unsupported router file")
      assert_includes(error.message, "pages/+bar.rb")
      assert_includes(error.message, "pages/+foo.haml")
      assert_includes(error.message, "pages/+page.svg")
    end
  end

  def test_accepts_supported_plus_haml_companions
    Dir.mktmpdir do |dir|
      FileUtils.mkdir_p("#{dir}/pages")
      File.write("#{dir}/pages/+page.haml", "")
      File.write("#{dir}/pages/+page.css", "")
      File.write("#{dir}/pages/+page.intl.en.toml", "")
