...
require_relative "test_plugin"

class Klenod::Build::Plugins::TestPlugin::Test < Minitest::Test
  TestPlugin = Klenod::Build::Plugins::TestPlugin
  TestSuite = Klenod::Build::TestSuite
  ...
  def test_application_modules_cannot_import_test_files
    ["./value.test.rb", "./value.test", "./value.test.rb"].each_with_index do |specifier, index|
      ...
      with_files(
        ...
        error = assert_raises(Klenod::Build::TestImportError) { context.collect("entry.rb") }
  ...
  def test_test_files_cannot_import_other_test_files
    with_files(
      ...
      assert_raises(Klenod::Build::TestImportError) { context.collect("one.test.rb") }
    end
  end
  ...
  def test_collect_indexes_dependencies_without_evaluating_modules
    Dir.mktmpdir do |dir|
      ...
      selection = TestSuite.new(context: context, plugin: plugin).collect
  ...
  def test_update_selects_tests_with_direct_transitive_and_shared_dependencies
    with_files(
      ...
      context, plugin = test_context(dir)
      suite = TestSuite.new(context: context, plugin: plugin)
      suite.collect
