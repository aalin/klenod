...
`GoogleFontsPlugin` and `RouterPlugin` are built in but opt-in. Application testing is provided separately by `klenod-test`.
...
- Bare method names in parenthesized event attributes are preserved as symbols, so `(on-change=handle_change)` passes `on_change: :handle_change` without calling the method during rendering. Set `event_handler` to a framework constant path to compile those references through its `callback(component, method_name)` method instead.
